...
async function initializeFirestoreFunctions() {
  ...
  // Here you can call other Firestore related functions
}
...
async function fetchData() {
  try {
    ...
    if (!transferreduserInfo || !transferredInfo) {
      throw new Error("Missing required localStorage values");
    }
    ...
    if (!reportSnap.exists()) throw new Error("Report document does not exist");
    if (!businessSnap.exists()) throw new Error("Business document does not exist");
    if (!usersSnap.exists()) throw new Error("User document does not exist");
    if (!contentSnap.exists()) throw new Error("Content document does not exist");
...
async function processData() {
 ...
 const Report = business.AppIcons.Report
  const HeaderIcons = data.Report
  const Users = data.User
  const Content = data.Content
 
  
  const { top, bottom } = business.BuColors.BackgroundColor;
  const { Base, Prime1, Prime2, Prime3, Prime4 } = business.BuColors.Colors;
  ...
  function extractNumber(str) {
    if (typeof str !== 'string') {
        console.error("Invalid value passed to extractNumber:", str);
        return 0; // safer fallback
    }
  ...
  renderBtnsContent()
  console.log(getScoring(age, gender, getFatPercent()))
