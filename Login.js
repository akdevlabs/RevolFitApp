...
import { getFirestore, doc, getDoc, updateDoc, serverTimestamp,Timestamp     } from "https://www.gstatic.com/firebasejs/9.1.1/firebase-firestore.js";
...
// Returns the current timestamp (Firebase format)
function getCurrentFirestoreTimestamp() {
  return Timestamp.now();
}
const currentTimestamp = getCurrentFirestoreTimestamp();
console.log("Firestore Timestamp:", currentTimestamp);
...
const formattedDate = convertFirestoreTimestampToDate(currentTimestamp);
console.log(formattedDate); // Outputs: "11/21/2025"
...
async function applyBranding(DId) {
  ...
  if (data) {
    ...
    if (Popup) {
      ...
      renderImage(data.BuLogos.Simple[3], "pop-upIcon");
    ...
    renderImage(data.BuLogos.Simple[3], "loginIcon");
